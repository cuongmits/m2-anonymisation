...
use Cuongmits\Anonymisation\Anonymiser\Entity\ShipmentTrack;
use Cuongmits\Anonymisation\Exception\AnonymisationException;
use Zend_Db_Expr;
use Cuongmits\Anonymisation\Anonymiser\Entity\Base;

final class ShipmentTrackSpec extends ObjectBehavior
{
    ...
    {
        $this->shouldHaveType(ShipmentTrack::class);
    }
    ...
    function it_should_process_a_single_quote_payment_with_all_bindings(
        AdapterInterface $connection,
        LoggerInterface $logger
    ) {
        $connection->update('sales_shipment_track', Argument::type('array'), ['order_id = ?' => 234])
            ->willReturn(1)
            ->shouldBeCalled();
    ...
    {
        $subject = $this->buildBindExpressions(['track_number' => 'xxxxx', 'tracking_url' => 'xxxxx']);

        $subject->shouldHaveCount(3);
        $subject->offsetGet('track_number')->shouldBeLike(
            new Zend_Db_Expr('IF(track_number_identifier IS NULL, NULL, xxxxx_value)')
        );
        $subject->offsetGet('tracking_url')->shouldBeLike(
            new Zend_Db_Expr('IF(tracking_url_identifier IS NULL, NULL, xxxxx_value)')
        );
    ...
    {
        $this->getTableName()->shouldReturn('sales_shipment_track');
    }

    function it_should_return_correct_entity_name()
    {
        $this->getEntityName()->shouldReturn('Order Shipment Track');
    }
