...
use Cuongmits\Anonymisation\Anonymiser\Entity\Shipment;
...
final class ShipmentSpec extends ObjectBehavior
{
    ...
    function it_should_process_a_single_quote_payment_with_all_bindings(
        ...
        $connection->update('sales_shipment', Argument::type('array'), ['order_id = ?' => 234])
            ->willReturn(1)
            ->shouldBeCalled();
    ...
    {
        $subject = $this->buildBindExpressions(['toom_invoice_id' => 'xxxxx']);
    ...
    {
        $this->getTableName()->shouldReturn('sales_shipment');
    }

    function it_should_return_correct_entity_name()
    {
        $this->getEntityName()->shouldReturn('Order Shipment');
    }
