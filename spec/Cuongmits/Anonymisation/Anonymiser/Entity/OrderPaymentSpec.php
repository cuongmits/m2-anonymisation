...
final class OrderPaymentSpec extends ObjectBehavior
{
    ...
    {
        $this->shouldHaveType(OrderPayment::class);
    }
    ...
    function it_should_process_a_single_order_payment_with_all_bindings(
        ...
        $connection->update('sales_order_payment', Argument::type('array'), ['parent_id = ?' => 234])
            ->willReturn(1)
            ->shouldBeCalled();
    ...
    {
        $this->getTableName()->shouldReturn('sales_order_payment');
    }

    function it_should_return_correct_entity_name()
    {
        $this->getEntityName()->shouldReturn('Order Payment');
    }
