...
final class QuoteSpec extends ObjectBehavior
{
    ...
    function it_should_processe_a_single_quote_with_all_bindings(
        AdapterInterface $connection,
        LoggerInterface $logger
    ) {
        $connection->update('quote', Argument::type('array'), ['entity_id = ?' => 234])
            ->willReturn(1)
            ->shouldBeCalled();
    ...
    function it_should_create_binds_for_anonymised_columns()
    {
        ...
        $subject->offsetGet('reserved_order_id')->shouldBeLike(new Zend_Db_Expr('IF(reserved_order_id_identifier IS NULL, NULL, CONCAT(SUBSTRING_INDEX(reserved_order_id_identifier, "-", 2), "-", entity_id_identifier))'));
    ...
    {
        $this->getTableName()->shouldReturn('quote');
    }

    function it_should_return_correct_entity_name()
    {
        $this->getEntityName()->shouldReturn('Quote');
    }
