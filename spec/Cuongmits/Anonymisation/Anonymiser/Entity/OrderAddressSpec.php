...
final class OrderAddressSpec extends ObjectBehavior
{
    ...
    {
        $this->shouldHaveType(OrderAddress::class);
    }
    ...
    function it_should_process_a_single_order_address_with_all_bindings(
        AdapterInterface $connection,
        LoggerInterface $logger
    ) {
        $connection->update('sales_order_address', Argument::type('array'), ['parent_id = ?' => 234])
    ...
    {
        $this->getTableName()->shouldReturn('sales_order_address');
    }

    function it_should_return_correct_entity_name()
    {
        $this->getEntityName()->shouldReturn('Order Address');
    }

    function it_should_return_correct_condition()
    {
        $this->getCondition()->shouldReturn('parent_id');
    }

    function it_should_return_correct_update_date_exists_status()
    {
        $this->updatedDateExists()->shouldReturn(false);
    }
