...
final class QuotePaymentSpec extends ObjectBehavior
{
    ...
    {
        $this->shouldHaveType(QuotePayment::class);
    }
    ...
    function it_should_processe_a_single_quote_payment_with_all_bindings(
        ...
        $connection->update('quote_payment', Argument::type('array'), ['quote_id = ?' => 234])
            ->willReturn(1)
            ->shouldBeCalled();
    ...
    {
        $this->getTableName()->shouldReturn('quote_payment');
    }

    function it_should_return_correct_entity_name()
    {
        $this->getEntityName()->shouldReturn('Quote Payment');
    }
